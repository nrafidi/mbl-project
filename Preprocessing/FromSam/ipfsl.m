...
thresh_file = [preprocdir 'brain_thresh.nii.gz'];
...
if ~exist(smooth_file,'file') || optInputs(varargin, 'overwrite')
    if actual_fwhm == 0
        copyfile([thresh_file],[smooth_file]);
    else
        % call susan
        unix_freesurfer_version(freesurfer_version, ['mri_fwhm --i ' thresh_file ' --o ' smooth_file ' --fwhm ' num2str(actual_fwhm) ' --smooth-only --mask ' mask_file]);
    end
end