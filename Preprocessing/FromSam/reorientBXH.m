    ...
    for j = 1:length(runtypes)
        ...
        for k = 1:length(runnum)
            ...
            if ~exist([lasfile '.nii.gz'],'file') || optInputs(varargin, 'overwrite');
                ...
                fprintf(['bxhreorient --orientation=LAS ' niifile '.bxh temp' id '.bxh \n']);
                unix(['bxhreorient --orientation=LAS ' niifile '.bxh temp' id '.bxh']);
                fprintf(['bxh2analyze -s --niigz temp' id '.bxh ' lasfile '\n']);
                unix(['bxh2analyze -s --niigz temp' id '.bxh ' lasfile]);
                fprintf(['rm temp' id '.* \n']);
                unix(['rm temp'  id '.*']);
            end
