...
if ~exist(exfunc2highres_regmat, 'file') || ~exist(highres2exfunc_regmat, 'file') || ~exist(exfunc2highres_nii, 'file') || optInputs(varargin, 'overwrite')
  % registration
  unix_fsl(fsl_version, ['flirt -in ' exfunc ' -ref ' highres ' -cost corratio -dof ' dof ' -searchrx ' search_range ' -searchry ' search_range ' -searchrz ' search_range ' -omat ' exfunc2highres_regmat]);
  % invert registration
